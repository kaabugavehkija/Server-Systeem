...
function registreeri(){
	if(isset($_POST['username'],$_POST['password'],$_POST['password2'])){
		...
		if(mysqli_stmt_fetch($stmt)){
			return "Sellenimeline kasutaja juba olemas!"; 
			exit;
		}else{
			if ($_POST['password']!= $_POST['password2']){
				return "Valitud paroolid ei olnud identsed!";
				}
			elseif (strlen($_POST['password'])<9){
				return ("Password peab olema pikem kui 8 tähemärki!");
			}
			else{
...
function kustuta(){
	...
	if(isset($_POST['kp'])){
		...
		header("location: ?page=detailvaade&ruumi_id=".$ruumi_id);
		exit();
		}
	}
?>
